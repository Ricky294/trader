from trader.core.util.trade import reduce_money_with_fee, calculate_money_fee, calculate_quantity_fee, \
...
def test_calculate_money_fee():
    ...
    # fee: 0%
    assert 0 == calculate_money_fee(quantity=quantity, price=price, fee_rate=.0, leverage=1)

    # fee: 1%, leverage: 1x
    assert 0.5 == calculate_money_fee(quantity=quantity, price=price, fee_rate=.01, leverage=1)

    # fee: 1%, leverage: 2x
    assert 2 == calculate_money_fee(quantity=quantity, price=price, fee_rate=.01, leverage=2)

    # fee 0.1%, leverage: 10x
    assert 5 == calculate_money_fee(quantity=quantity, price=price, fee_rate=.001, leverage=10)

    # fee: 1%, leverage: 10x
    assert 50 == calculate_money_fee(quantity=quantity, price=price, fee_rate=.01, leverage=10)
